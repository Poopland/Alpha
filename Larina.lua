    ...
    function Window:AddTab(Name)
        ...
        table.insert(Library.Signals, TabFrame.ChildAdded:Connect(function(v)
            table.insert(Library.Signals, v.ChildAdded:Connect(function(v)
                if v:IsA("Frame") then
                    table.insert(UIalist, v)
                    table.insert(Library.Signals, v:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
                        if v.AbsoluteSize.Y > TabFrame.CanvasSize.Y.Offset then
                            TabFrame.CanvasSize = UDim2.fromOffset(0,v.AbsoluteSize.Y + 10)
                        end
                    end))
                end;
            end))
        end))
        while Library.Load and wait(1) do
            for i,v in pairs(UIalist) do
                if v.AbsoluteSize.Y > TabFrame.CanvasSize.Y.Offset then
                    TabFrame.CanvasSize = UDim2.fromOffset(0,v.AbsoluteSize.Y + 10)
                end
            end
